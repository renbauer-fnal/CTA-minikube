...
mkdir -p /opt/mhvtl
groupadd vtl || true
adduser vtl -g vtl || true
make_vtl_media -C /etc/mhvtl
...
sudo -u cta ./generate_librarydevice_PV.sh

################################################################################
### puppet:///modules/hg_cta/generate_PV.sh
echo Generating the log persistent volume
mkdir -pv /shared/logs
sudo -u cta kubectl delete pv log || true
sudo -u cta kubectl create -f ./log_PV.yaml
sudo -u cta kubectl get persistentvolumes -l type=logs

echo Generating the stg persistent volume
rm -rf /shared/stg
mkdir -pv /shared/stg
chown cta /shared/stg
sudo -u cta kubectl delete pv stg || true
sudo -u cta kubectl create -f ./stg_PV.yaml
sudo -u cta kubectl get persistentvolumes -l type=stg
