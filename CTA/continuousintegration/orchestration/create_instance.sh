...
for ((i=0; i<400; i++)); do
  echo -n "."
  kubectl get pod init --namespace=${instance} | egrep -q 'Completed|Err' && break
  sleep 1
done
...
if $(kubectl get pod init --namespace=${instance} | grep -q Error); then
	...
	die "ERROR: init pod in Error state. Initialization failed."
fi
...
if [ $runoracleunittests == 1 ] ; then
  ...
  for ((i=0; i<400; i++)); do
    echo -n "."
    kubectl get pod oracleunittests --namespace=${instance} | egrep -q 'Completed|Err' && break
    sleep 1
  done
