...
'''Command line tool for the actual copy of a tapepool's metadata from CASTOR to CTA'''
...
def usage(exitcode):
    '''prints usage'''
    print __doc__
    print 'Usage : ' + sys.argv[0] + ' [-h|--help] -t|--tapepool <tapepool> -v|--vo <VO> -i|--eosctainstance <eosctainstance> --dryrun|--doit'
    sys.exit(exitcode)
...
def async_import(conn, tapepool, vo, eosctainstance, dryrun):
...
def run():
    ...
    try:
        options, _ = getopt.getopt(sys.argv[1:], 'ht:v:i:d', ['help', 'tapepool=', 'vo=', 'eosctainstance=', 'dryrun', 'doit'])
    except Exception, e:
        print e
        usage(1)
    for f, v in options:
    ...
    # deal with arguments
    if not tapepool or not vo or not eosctaInstance or dryrun == None:
    ...
    try:
        ...
        runner = Thread(target=async_import, args=(ctaconn, tapepool, vo, eosctaInstance, dryrun))
        runner.start()
